...
我们能够在[标准](https://en.cppreference.com/w/cpp/language/derived_class)中找到关于虚继承的描述。需要注意的是，标准仅规范的是行为却不包含实现，故，如果要上升到如何实现这一特性的任何细节，这往往需要视编译器而定。然而在这个过程中有两个大家普遍所认可的概念：***vbptr***（$Virtual\ Base\ Pointer$）和 ***vbtable***（$Virtual\ Base\ Table$）。

虚派生类被构造的时候，编译器会把从包含的虚派生类首地址开始到虚基类首地址所需偏移量都整合到一个名为 vbtable 的表格中。这个特殊的表格作为绑定到虚派生类自身的一个属性，它允许相同类类型的所有实例间共享。请注意，这并不是一种抽象的概念，vbtable 会实际存在于进程运行时的内存中，其大小需要视虚基类中所声明的成员数量而定。为了保证安全性，没有任何理由允许这块表格所在的内存段拥有可写属性，因此或许放在一个静态常量区中或许是一个合理的实现（在 GCC 的实现中会将 vbtable 放置在 data.rel.ro 段当中）。

当一个类类型拥有了 vbtable 的绑定后，对象的内存模型则发生了改变，参考[继承的内存模型](#继承的内存模型)中的表述，（虚）基类的数据不再初始化在实例内存的起始部分，相反，它会被放置在实例内存的结尾部分。另外一点，对于虚基类中的成员引用方式相较于原始的固定静态偏移退化到两个步骤：首先，依据目标成员符号从 vbtable 中找到重定位所需的偏移；其次，取得这个偏移加上实例自身在内存首地址的结果，将其作为成员地址的最终索引。

到这里，我们还剩下一份工作没做，即虚派生类要如何在语言的运行时信息过少情况下知道绑定的 vbtable 呢？特别是，指针类型的引入而又为这个问题增加了更多的复杂性。为解决这一问题，现代编译器引入了一个指向 vbtable 的指针，即 vbptr。前面描述到，作为虚基类会被反向初始化到实例内存的尾部，而那一部分空余出来的内存部分则使用 vbptr 替代。值得注意的是，即使是相同类型的多个实例，其 vbptr 都是恒等且指向一个唯一的 vbtable。这样的设计不仅确保每个实例都能访问到正确的虚函数实现，而且为实现 RTTI（$Run-Time\ Type\ Information$）提供了坚实的基础，使得我们可以在运行时安全地识别和处理对象的实际类型。虽然在某种程度上存在一部分冗余，但是产生的好处促使编译器更愿意接受这样的成本。

<div class = "center">

```text
class Hybird    size(20):
        +---
 0      | +--- (base class Dog)
 0      | | {vbptr}
 4      | | m_val_dog
...
Hybird::$vbtable@Dog@:
 0      | 0
 1      | 16 (Hybirdd(Dog+0)Animal)

Hybird::$vbtable@Cat@:
 0      | 0
 1      | 8 (Hybirdd(Cat+0)Animal)
...
考虑一个场景：我们有一个复杂的继承层次结构，类 B 和 C 都虚拟继承自类 A，类 D 则继承自 B 和 C。假设我们有一个函数，它需要 B* 类型的参数，并返回 A* 类型的参数。函数的输入参数必须能够接受任何指向 B 对象的指针，这包括了 D 对象，抑或是其他派生自 B 的对象。当我们传入一个由 D 构造的指针时，索引到 A 子对象需要经过三条路径；如果我们传入的指针由 B 所构造呢？差异性在这里就体现出来了。所以必须提供一种机制，使得含有 B* 的代码能找出 A 子对象的位置。这完全依赖于最终派生类型的继承层级，因此 vptr/vtable 确实是一种合理的机制。
...
虚函数是[多态](#里式转换)的一种实现定义，如果一个基类的指针(引用)所指向的是其派生实例在内存中的首地址并且该派生对于基类中的虚函数进行了重写，那么当我们使用这个基类的指针(引用)调用目标虚函数时，所调用的是派生类中的重写实现。
...
编译器会将类中声明的所有虚函数的实际地址组织起来并放入到一个名为 vftable 的表格中，我们可以将这个特殊的表格视为类本身的一个属性，也就是说，它会绑定到一个具体的类型，且允许相同类型的所有实例间共享。vftable 会实际存在于进程运行时的内存中，其大小需要视类型所声明的虚函数的数量而定，然而具体到 vftable 所在的内存区域的问题，由于标准中并不包含这项实现因此我们也无法完全确定，但为了保证安全性，没有任何理由允许这块表格所在的内存段拥有可写属性，因此或许放在一个静态常量区中是一个合理的实现（在 GCC 的实现中会将 vftable 放置在 `data.rel.ro` 段当中）。

一份出自基类中初始状态下（承载着基类中所有虚函数实际地址）的 vftable 会沿着继承链往下传递，派生类会将传递下来的 vftable 拷贝一份并将这份新的 vftable 绑定到自身类型，随后对其进行修改：如果派生类重写（override）了任何虚函数，相应的函数地址会在 vftable 的某个表项中的反应到这种修改。此过程会在每一级派生类中重复进行，直至继承链的末端。每一级的派生类都会根据自身重写的需要更新和调整继承来自直接基类的 vftable 的拷贝，确保在对象的方法调用时，能够正确地引用到最新的实现。

当我们完成了这段 vftable 与类型之间的绑定工作后，实现多态就变得非常直观。每一个实例对应的类型中都拥有一份唯一且从根基类所继承并交由后续派生修改的 vftable。这样，在运行时当我们调用一个虚函数，程序就可以通过当前实例的类型对应的 vftable 结合具体的偏移量来动态确定应调用哪个函数实现。这个动态决定的过程就是多态的实现核心。

但请不要过于乐观，CPP 是一种较低级别且原生的语言，在运行时它仅包含有限的类型信息。那么如何查找一个实例所对应的类型则成为了一个很大的问题，特别是指针的引入而又增加了此问题更多的复杂性。为解决这一问题，现代编译器引入了一个指向 vftable 的指针，即 vfptr。当一个类被实例化的时候，如果编译器检测到该类含有虚成员，那么就会在其构造后的内存结构的首部中插入一个指向与当前类型绑定的 vftable 的指针 vfptr，在完成这样的准备工作后，编译器就能够非常方便的在实例（某个类型）调用一个虚函数前去查询该实例所对应的 vftable 以完成最终函数寻址的工作。值得注意的是，即使是相同类型的多个实例，其 vfptr 都是恒等且指向一个唯一的 vftable。这样的设计不仅确保每个实例都能访问到正确的虚函数实现，而且为实现 RTTI（$Run-Time\ Type\ Information$）提供了坚实的基础，使得我们可以在运行时安全地识别和处理对象的实际类型。虽然在某种程度上存在一部分冗余，但是产生的好处促使编译器更愿意接受这样的成本。
